...
public class DeviceSetContentProvider extends ContentProvider {
    ...
    private DatabaseDevicesSetConnectHelper mOpenHelper;
    ...
    public boolean onCreate() {
        this.mOpenHelper = new DatabaseDevicesSetConnectHelper(getContext());
        String str = getContext().getPackageName();
        sUriMatcher.addURI(str, DevicesSetColumns.TABLE_NAME, DEVICES);
        sUriMatcher.addURI(str, "devicesset/#", DEVICESID);
        DevicesSetProvider.setUriString(str);
        return true;
    }
    ...
    public String getType(Uri uri) {
        switch (sUriMatcher.match(uri)) {
            case DEVICES /*1*/:
                return DevicesSetProvider.CONTENT_TYPE;
            case DEVICESID /*2*/:
                return DevicesSetProvider.CONTENT_ITEM_TYPE;
            default:
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
        ...
        switch (sUriMatcher.match(uri)) {
            ...
            case DEVICESID /*2*/:
                String orderBy;
                qb.setTables(DevicesSetColumns.TABLE_NAME);
    ...
    public Uri insert(Uri uri, ContentValues initialValues) {
        ...
        switch (sUriMatcher.match(uri)) {
            case DEVICES /*1*/:
                tableName = DevicesSetColumns.TABLE_NAME;
    ...
    public int delete(Uri uri, String where, String[] whereArgs) {
        ...
        switch (sUriMatcher.match(uri)) {
            case DEVICES /*1*/:
                int count = db.delete(DevicesSetColumns.TABLE_NAME, where, whereArgs);
                getContext().getContentResolver().notifyChange(uri, null);
                return count;
            case DEVICESID /*2*/:
                db.delete(DevicesSetColumns.TABLE_NAME, "_id=" + ((String) uri.getPathSegments().get(DEVICES)) + (!TextUtils.isEmpty(where) ? " AND (" + where + ')' : ""), whereArgs);
                break;
        }
        throw new IllegalArgumentException("Unknown URI " + uri);
    }
    ...
    public int update(Uri uri, ContentValues values, String where, String[] whereArgs) {
        ...
        switch (sUriMatcher.match(uri)) {
            case DEVICES /*1*/:
                count = db.update(DevicesSetColumns.TABLE_NAME, values, where, whereArgs);
                break;
            case DEVICESID /*2*/:
                count = db.update(DevicesSetColumns.TABLE_NAME, values, "_id=" + ((String) uri.getPathSegments().get(DEVICES)) + (!TextUtils.isEmpty(where) ? " AND (" + where + ')' : ""), whereArgs);
                break;
            default:
                throw new IllegalArgumentException("Unknown URI " + uri);
        }
